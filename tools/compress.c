...
char *const code_attr[ALPHABET_SIZE][2] =
{
{"eClass","a"},
{"name","aa"},
{"values","ab"},
...
{
  // Compress model
  FILE *in  = fopen("../models/7nodes1component.json", "r");
  FILE *out = fopen("7nodes1component.compress.json", "a");
  substitutionCode(in, out, COMPRESS);
  fclose(in);
  fclose(out);

  // Decompress model
  in  = fopen("7nodes1component.compress.json", "r");
  out = fopen("7nodes1component.decompress.json", "a");
